...
* Confidence level: 5/5 with **over-gathering** 

## What else? (1)
```{r}
names(df)
```
*  Varibles' name are not consistent.
...
* X__1 contains **id** and **group** (NS & NNS).
    + Confidence level: 2/5 with `tidyverse::separate`
        - sep = ?
    
* Duplicate id number for NS and NNS
    + Confidence level: 0/5 
        - use `tidyverse::case_when` ?
...
```
* Couple issues:
    + 1. X and Y axes need to be rotated.
    + 2. Labels are not in a conventional order.
